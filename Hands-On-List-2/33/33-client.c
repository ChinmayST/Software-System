/*
============================================================================
Name : 33-client.c
Author : Chinmay Tavarej
Description : Write a program to communicate between two machines using socket.
Date: 10th Oct , 2024
============================================================================
*/

// This is the client program

/*
    1. socket() -> create an endpoint for the communication
        sockfetfd = socket(AF_INET, SOCK_STREAM, 0);
    2. connect() -> initiate a connection to a socket
        connectionStatus = connect(socketfd, &socketaddr, sizeof(socketaddr));
    3. Start communicating -> `write` to and `read` from socketfd
*/

#include <sys/types.h>  // Import for `socket`, `connect`
#include <sys/socket.h> // Import for `socket`, `connect`
#include <netinet/ip.h> // Import for `struct sockaddr_in`, `htons`
#include <stdio.h>      // Import for `perror` & `printf`
#include <unistd.h>     // Import for `_exit`, `read`, write`

void main()
{
    int socketFileDescriptor; // File descriptor that will be used for communication via socket . number used to identify socket.
    int connectStatus;        // Determines success of `connect` call

    struct sockaddr_in address; // Holds the address of the server to communicate

    ssize_t readBytes, writeBytes; // Number of bytes read from / written to the network via socket
    char dataFromServer[100];

    // Create an endpoint for communicate -> here, create the client side point
    // Create a socket for TCP connection using IPv4
    socketFileDescriptor = socket(AF_INET, SOCK_STREAM, 0);  //AF_INET means using IPV4 , SOCK_STREAM means using TCP
    if (socketFileDescriptor == -1)
    {
        perror("Error while creating socket!");
        _exit(0);
    }
    printf("Client side socket successfully created!\n");

    // Defined server's details
    address.sin_addr.s_addr = htonl(INADDR_ANY);   //INADDR_ANY : wants to connect to any available server
    address.sin_family = AF_INET;             //sin_Family specifies address fam (ipv4)
    address.sin_port = htons(8080);           //port number to 8080 where the server listens for connections

    connectStatus = connect(socketFileDescriptor, (struct sockaddr *)&address, sizeof(address));
    if (connectStatus == -1)
    {
        perror("Error while connecting to server!");
        _exit(0);
    }
    printf("Client to server connection successfully established!\n");

    // ========================= Client - Server communication =================

    readBytes = read(socketFileDescriptor, dataFromServer, 100);
    ;
    if (readBytes == -1)
        perror("Error while reading from network via socket!");
    else
        printf("Data from server: %s\n", dataFromServer);

    writeBytes = write(socketFileDescriptor, "I'm the client!", 15);
    if (writeBytes == -1)
        perror("Error while writing to network via socket!");
    else
        printf("Data sent to server!");

    // =======================================================================

    close(socketFileDescriptor);
}

/*
============================================================================
Output : 
On 1st Terminal : 
iiitb@iiitb-Dell-G15-5511:~/Hands-On-List-2/33$ gcc 33-client.c -o 33-client
iiitb@iiitb-Dell-G15-5511:~/Hands-On-List-2/33$ gcc 33-server.c -o 33-server
iiitb@iiitb-Dell-G15-5511:~/Hands-On-List-2/33$ ./33-server
Server side socket successfully created!
Binding to socket was successful!
Now listening for connections on a socket!
Data sent to client!
Data from client: I'm the client!

On 2nd terminal : 
iitb@iiitb-Dell-G15-5511:~/Hands-On-List-2/33$ ./33-client
Client side socket successfully created!
Client to server connection successfully established!
Data from server: I'm the server!
Data sent to server!


============================================================================
*/ 
